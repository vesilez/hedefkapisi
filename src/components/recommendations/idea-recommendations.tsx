...
export function IdeaRecommendations({ role }: { role: MatchRole }) {
  ...
  if (error) return (
    <Card className="mt-8 border-red-200 bg-red-50 text-center">
      <p className="font-semibold text-red-800" role="alert">{error}</p>
      <Button className="mt-4" variant="secondary" onClick={() => void load()}>Tekrar Dene</Button>
    </Card>
  );
          ...
          {items.map(({ idea, score, reasons }) => (
            <Card key={idea.id} className="flex h-full flex-col">
              ...
              <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
                <div><dt className="font-semibold text-slate-500">Kategori</dt><dd className="text-slate-800">{DEFAULT_CATEGORIES.find((category) => category.id === idea.categoryId)?.label ?? idea.categoryId}</dd></div>
                <div><dt className="font-semibold text-slate-500">Şehir</dt><dd className="text-slate-800">{idea.city ?? "Belirtilmedi"}</dd></div>
                <div className="sm:col-span-2"><dt className="font-semibold text-slate-500">İhtiyaç duyulan destekler</dt><dd className="mt-1 flex flex-wrap gap-1">{idea.supportNeeds.map((support) => <Badge key={support}>{SUPPORT_TYPE_LABELS[support]}</Badge>)}</dd></div>
              </dl>
              ...
              <Link href={`/hayaller/${idea.slug}`} className="mt-auto pt-5 font-semibold text-blue-700">Detayları Gör →</Link>
            </Card>
          ))}
        </div>
      )}
